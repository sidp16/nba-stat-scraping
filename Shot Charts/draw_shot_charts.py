...
def getShotChartDetails(player_name, season_id):
    # player dictionary
    ...
    return shotchartlist[0], shotchartlist[1], player_name, season_id
...
def shot_chart(data, title="", color="b", xlim=(-250, 250), ylim=(422.5, -47.5), line_color="blue",
    ...
    totalMissed = len(x_missed)

    x_made = data[data['EVENT_TYPE'] == 'Made Shot']['LOC_X']
    y_made = data[data['EVENT_TYPE'] == 'Made Shot']['LOC_Y']
    totalMade = len(x_made)
...
if __name__ == "__main__":
    ...
    player_shotchart_df, league_avg, player_name, season_id = getShotChartDetails(player_name="Tim Duncan", season_id="2013-14")
    
    # title
    title = player_name + "'s Shot Chart " + "in " + season_id

    # Draw Court and plot Shot Chart
    totalMade, totalMissed = shot_chart(player_shotchart_df, title=title)
    # Set the size for our plots
    plt.rcParams['figure.figsize'] = (12, 11)
    plt.xlabel(f"{totalMade} shots made & {totalMissed} shots missed!", fontsize="xx-large")
    plt.savefig(f"{player_name} in {season_id}", dpi=300)
    plt.show()